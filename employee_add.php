...
<?php
    ...
    else
	{
		...
		if(isset($_GET['bn']) && intval($_GET['bn']) <= $number_of_row && intval($_GET['bn'] != 0 )) {
			...
			$sql = mysqli_query($db, "SELECT * FROM employee WHERE RoleId !='1' LIMIT $Skip, $recordLimit");
		} else {
			$sql = mysqli_query($db,"SELECT * FROM employee WHERE RoleId !='1' LIMIT $recordLimit");
		}
